...
public partial class time_sheet : System.Web.UI.Page
{
    ...
    private void tasksDropDownInitializer()
    {
        ...
        else if (RadioButtonListTaskType.SelectedIndex == 1)
        {
            DropDownListTaskType.DataSource = AppUsers.JobTasks();
        }
        else
        {
            DropDownListTaskType.DataSource = AppUsers.MaintenanceTasks();
        }
    ...
    protected void ButtonSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (dateOfWork == DateTime.Today || dateOfWork > weekly)
        {
            if ((bool)(Session["RecordInserted"]) == true)
            {
                try
                {
                    EmployeeTimeRecordMexicoTableAdapter etrAdapter = new EmployeeTimeRecordMexicoTableAdapter();
                    ...
                    etrAdapter.Insert(PID, UserID, this.employeeName, RoleName, JobNum, JobDescription, TaskType, Date, Hours, Minutes, Plant, Comments);
